        ...
        message *m_ptr;
{
    struct mproc *z;
    int CS_PROC_NR=-1;
    for (z = &mproc[0]; z < &mproc[NR_PROCS]; z++) {
        if (!(z->mp_flags & IN_USE))
            continue;
        if (!strcmp(z->mp_name, "cs")) {
            CS_PROC_NR = z->mp_endpoint;
            break;
        }
    }
    if(CS_PROC_NR == -1)
    ...
    int r = sendnb(CS_PROC_NR, m_ptr);
    if (r != OK)
        printf("unable to send to CS: %d", r);
}